...
function frequentVisitors()
{
	local histogramOutput=$(histogram)
	local frequentVisitorsOutput=$(echo "$histogramOutput" | awk 'int($0)>=10')
	echo "$frequentVisitorsOutput"
}
...
function suspiciousVisitors()
{
	local suspiciousVisitorsOutput=$(grep -f "IOC.txt" "$logFile")
	echo "$suspiciousVisitorsOutput" | cut -d' ' -f 1 | sort | uniq -c
}
...
do
	...
	# Display a message, if an invalid input is given
		...
		echo "Invalid input. Please select an option between 1 and 7"
	fi
done
